    ...
    for tc in tool_calls:
        ...
        if "app.py" in str(target):
            ...
            if name == "replace_file_content":
                ...
                print(f"  TargetContent size={len(args.get('TargetContent', ''))}, ReplacementContent size={len(args.get('ReplacementContent', ''))}")
            elif name == "multi_replace_file_content":
                chunks = args.get("ReplacementChunks", [])
                print(f"  multi_replace with {len(chunks)} chunks")
                for i, chunk in enumerate(chunks):
                    print(f"    Chunk {i}: StartLine={chunk.get('StartLine')}, EndLine={chunk.get('EndLine')}, TargetContent size={len(chunk.get('TargetContent', ''))}, ReplacementContent size={len(chunk.get('ReplacementContent', ''))}")
